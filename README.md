...
在做机器人项目时经常遇到这种情况：Jetson Orin 装在机身内部，深度相机等 USB 3.0 外设需求要装在外壳或机械臂末端，用户接口需要通过转接线 + 接口板的方式对外输出。多次转接+外接线缆过长，5 Gbps 的 SuperSpeed 信号衰减到链路训练失败，主机自动回落到 USB 2.0 —— 设备能识别，但跑不动 USB 3.0。
...
> **这块板的定位**：单通道 + 8 位拨码，是用来摸索 EQ / DE 增益配置组合的**标定板**，不是最终装机形态 —— 设计取舍见 [4.1 节「设计意图」](#design-intent)。
...
四路版本（同工程 V0.7）是往整机里装的，但用它试增益配置很别扭：四路同时工作，改一次配置要动 4 套电阻，通道之间还有串扰和供电耦合，测出结果说不清是哪一路的功劳。

单通道把变量压到最少 —— 一条链路、一颗芯片、四个配置脚。这样测出的结论是干净的，可以直接固化到四路版上。

**为什么用拨码而不是固定电阻**

V0.7 四路版用的是固定电阻 pinstrap（原理图里标 `n'c'c` 的位号就是不贴）。这套做法定型后没问题，摸索阶段却很痛苦：**每试一组 EQ/DE 就要换一次料**，一次实验周期等于一次焊接周期，光对比几组配置就要焊一下午。
...
> **复刻提示**：目标是"装到机器里用"的话，直接抄 V0.7 四路版、或把本板的配置固化成电阻更合适。这块板的价值在于——在动手做最终版本之前，先把增益配置这块的不确定性测清楚。
